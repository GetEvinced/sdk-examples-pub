# Evinced Cypress JS SDK Examples

Accessibility testing examples using the [Evinced Cypress SDK](https://developer.evinced.com/sdks-for-web-apps/cypress-sdk) with JavaScript.
...
Credentials are picked up in `cypress/support/e2e.js` via `Evinced.setCredentials()`.
...
```bash
npx cypress run --spec "cypress/e2e/evAnalyze.cy.js"
```
...
|------|---------|-------------|
| `evAnalyze.cy.js` | Single scan | Calls `cy.evAnalyze()` to scan the page and asserts on issue count |
| `evStartStop.cy.js` | Continuous scan | Uses `cy.evStart()` / `cy.evStop()` to capture DOM changes across interactions; shows opt-in platform upload |
| `evHooks.cy.js` | Framework hooks | `beforeEach`/`afterEach` wrap multiple tests with continuous scanning and labels |
| `evSaveFile.cy.js` | Save reports | Saves scan results to HTML and JSON using `cy.evSaveFile()` |
| `evFailIfCritical.cy.js` | Severity filter | Filters issues by severity and asserts that no critical issues exist |
| `evTestLink.cy.js` | Platform link | Retrieves the Evinced Platform URL for the uploaded report via `cy.evGetUploadTestUrl()` |

## SDK API summary

| Method | Description |
|--------|-------------|
| `Evinced.init(options?)` | Initialize once in `cypress/support/e2e.js` |
